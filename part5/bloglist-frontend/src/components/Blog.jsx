  ...
  return (
    ...
    <div style={containerStyle} className="blogEntry">
      ...
      <div style={entryStyle}>
        <div style={titleStyle}>{blog.title}</div><div style={byStyle}>by</div><div style={authorStyle}>{blog.author}</div>
        <button onClick={toggleVisibility}>{buttonLabel}</button>
      </div>

      <div style={showWhenVisibleStyle} className="expandedBlog">
        <div>{blog.url}</div>
        <div id="likeCounter">{blog.likes}<button onClick={addLike}>like</button></div>
